...
class ConsolidationNetwork(nn.Module):
    def __init__(
        ...
        self.U = torch.from_numpy(
            (np.random.randn(nneurons, nganglia) / np.sqrt(nneurons)).astype(np.float32)
        ).to(device)
        self.V = torch.from_numpy(
            (np.random.randn(nganglia, nneurons) / np.sqrt(nneurons)).astype(np.float32)
        ).to(device)
        self.B_m1 = nn.Parameter(
            torch.from_numpy(np.random.randn(nneurons, 1).astype(np.float32)).to(device)
        )
        self.B_bg = nn.Parameter(
            torch.from_numpy(np.zeros(nganglia).astype(np.float32)).to(device)
        )
        self.Wout = nn.Parameter(
            torch.from_numpy(
                (np.random.randn(1, nneurons) / np.sqrt(nneurons)).astype(np.float32)
            ).to(device)
        )
        self.I_go = nn.Parameter(
            torch.from_numpy(np.random.randn(nneurons, 1).astype(np.float32)).to(device)
        )
        ...
        **kwargs,
        ...
        if amplitude_modulated:
            for idx, pulse in enumerate(pulse_times):
                amplitudes = amplitude * np.sin(
                    (times - pulse - delay) * (frequency / 2) * self.dt
                )
                targets[:, idx] = amplitudes * np.sin(
                    (times - pulse - delay) * frequency * self.dt
                )
    ...
    def forward(
        ...
        assert (
            targets.shape[1] == triggers.shape[0]
        ), "The number of targets must be equal to the number of go cues."
        device = self.device
        batch_size = targets.shape[1]
        Targets = torch.from_numpy(targets.astype(np.float32))
        go_cues = self.Pulses[:, triggers]
        position_store = torch.zeros(self.total_time, batch_size)
        xm1 = torch.randn((self.J.shape[0], batch_size), device=device) / np.sqrt(
            self.J.shape[0]
        )
        ...
        for ti in range(self.total_time):
            xm1 = xm1 + self.dt / self.tau * (
                -xm1
                + (self.J + self.U @ rthal @ self.V) @ rm1
                + self.B_m1
                + self.I_go * go_cues[ti]
                + torch.randn(rm1.shape, device=device)
                * np.sqrt(2 * noise_scale**2 * (self.tau / self.dt))
            )
            rm1 = self.neural_nonlinearity(xm1)
    ...
    def plot_different_gains(self, trigger: int = 0, save_path: str = None):
        ...
        for idx, gain in enumerate(gains):
            ...
            xm1 = torch.randn(
                (self.J.shape[0], gains.shape[0]), device=device
            ...
            plt.plot(position_store[:, idx].detach().cpu(), alpha=0.5)
        plt.legend()
    ...
    def plot_variances(self, trigger: int = 0, samples: int = 100):
        ...
        for idx in range(samples):
            rthal = torch.diag(
                torch.randn(self.B_bg.shape, device=device) * magnitude + self.B_bg
            )
            xm1 = torch.randn((self.J.shape[0], samples), device=device) / np.sqrt(
                self.J.shape[0]
            )
            rm1 = self.neural_nonlinearity(xm1)
            for ti in range(self.total_time):
    ...
    def fit_bg_loop(
        ...
        frequency: float = 1,
        amplitude: float = 1,
        amp_modulated: bool = False,
        save_path: str = None,
        **kwargs,
    ):
        if not bg_rank is None:
            self.bg_rank = bg_rank
        self.new_targ_params = {
            "frequency": frequency,
            "amplitude": amplitude,
            "amplitude_mod": amp_modulated,
        }
        self.set_thalamic_weights(g1=g1, g2=g2)
        self.create_targets(
            frequency=frequency, amplitude=amplitude, amplitude_modulated=amp_modulated
        )
        ...
        if make_plots:
            ...
            for g in np.linspace(-1, 1):
                j_lr = (
                    ...
                    @ np.diag(g * self.B_bg.detach().cpu().numpy())
    ...
    def set_thalamic_weights(self, g1: float = 0, g2: float = 0):
        ...
        self.U = torch.from_numpy(
            (
                (np.sqrt(1 - g1**2)) * U[:, : self.bg_rank]
                + g1**2 * np.random.randn(J.shape[0], self.bg_rank)
            ).astype(np.float32)
        ).to(device)
        self.V = torch.from_numpy(
            (
                (np.sqrt(1 - g2**2)) * Vh[: self.bg_rank]
                + g2**2 * np.random.randn(self.bg_rank, J.shape[0])
            ).astype(np.float32)
        ).to(device)
        self.B_bg = nn.Parameter(
            torch.from_numpy(np.zeros(self.bg_rank).astype(np.float32)).to(device)
        )
        return None
...
if __name__ == "__main__":
    ...
    trial_types = [
        "frequency",
        "amplitude",
        "freqency_and_amplitude",
        "amplitude_modulated",
    ]
    param_values = [
        {"frequency": 2, "trials": 500},
        {"amplitude": 2.5, "trials": 500},
    ...
    for bg_rank in [25, 50, 75, 100]:
        for g1, g2 in itertools.product(g1s, g2s):
            ...
            for trial_type, params in zip(trial_types, param_values):
                ...
                included_files = [
                    file.stem
                    for file in all_subfolders
                    if re.search(reg_exp, file.stem)
                ]
                ...
                model.to(device)
                model.fit_bg_loop(
                    g1=g1, g2=g2, bg_rank=bg_rank, save_path=str_path, **params
                )
