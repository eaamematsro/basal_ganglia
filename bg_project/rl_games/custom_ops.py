...
class ContinuousPPO(nn.Module):
    def __init__(
        ...
        **hyperparameters,
        ...
        self.actor_logstd = nn.Parameter(torch.zeros(1, self.act_dim).to(self.device))
        actor_params = list(self.actor.parameters())
        actor_params.append(self.actor_logstd)
    ...
    @staticmethod
    def make_env(gym_id, idx, seed, capture_videos: bool = True, rec_freq: int = 5000):
        def thunk():
            env = gymnasium.make(gym_id, render_mode="rgb_array")
            env = gymnasium.wrappers.RecordEpisodeStatistics(env)
            if capture_videos:
    ...
    def get_action_and_value(self, obs, action=None):
        ...
        stds = self.actor_logstd.expand_as(means).exp()
        probs = MultivariateNormal(means, torch.diag_embed(stds))
        if action is None:
            action = probs.sample()
