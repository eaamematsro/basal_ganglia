...
class RNNStaticBG(BaseArchitecture):
    def __init__(self, nneurons_rnn: int = 100, nbg: int = 20, rnn_nfn: Optional[nn.Module] = None,
                 g0: float = 1.2, rnn_input_sources: Optional[Dict[str, Tuple[int, bool]]] = None,
        ...
        self.rnn = ThalamicRNN(nneurons=nneurons_rnn, nbg=nbg, non_linearity=rnn_nfn, g0=g0,
                               input_sources=rnn_input_sources, dt=dt, tau=tau, device=device)
        self.bg = MLP(layer_sizes=bg_layer_sizes, non_linearity=bg_nfn, input_size=bg_input_size,
                      output_size=nbg)

    def forward(self, bg_inputs: torch.Tensor,
